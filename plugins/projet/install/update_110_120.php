...
function update110to120() {
   ...
   if (FieldExists("glpi_plugin_projet_tasks", "suppliers_id")) {
      $query = "ALTER TABLE `glpi_plugin_projet_tasks` 
   ...
   if (!empty($tasks)) {
      foreach ($tasks as $task) {
         ...
         $input["plugin_projet_tasks_id"]=$task["id"];
         $input["begin"]=$task["date_begin"];
         $input["end"]=$task["date_end"];
         $PluginProjetTaskPlanning->add($input);
         unset($input);
      }
   }
