...
class PluginProjetTask_Item extends CommonDBTM {
   ...
   function canView() {
	...
	function addTaskItem($plugin_projet_tasks_id,$itemtype,$items_id) {

      $this->add(array('plugin_projet_tasks_id'=>$plugin_projet_tasks_id,'items_id'=>$items_id,'itemtype'=>$itemtype));
   ...
   function showItemFromPlugin($instID,$withtemplate='') {
      ...
      if ($PluginProjetTask->getFromDB($instID)) {
               ...
               if ($item->canView()) {
                  ...
                  if ($DB->numrows($result_linked)) {
                     ...
                     while ($data=$DB->fetch_assoc($result_linked)) {
                        ...
                        if($canedit && $this->canCreate() && $withtemplate<2)
                           echo "<td class='center tab_bg_2'><a href='./task.form.php?deletetaskitem=deletetaskitem&amp;id=".$data["items_id"]."'>".$LANG['buttons'][6]."</a></td>";
                        else echo "";
                        echo "</tr>";
                     }
                  }
         ...
         if($canedit && $this->canCreate() && $withtemplate<2) {
            ...
            echo "<input type='submit' name='addtaskitem' value=\"".$LANG['buttons'][8]."\" class='submit'>";
            echo "</td></tr>";
            echo "</table></div>" ;
