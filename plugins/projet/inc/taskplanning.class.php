   ...
   function showFormForTask($projet, PluginProjetTask $task) {
      ...
      if ($taskid>0 && $this->getFromDBbyTask($taskid)) {
         if ($this->canCreate()) {
            ...
            ajaxUpdateItemJsCode('viewplan', $CFG_GLPI["root_doc"] . "/plugins/projet/ajax/planning.php", $params);
            echo "}";
            echo "</script>\n";
...
//                   if (haveAccessToEntity($job->fields["entities_id"])) {
                     ...
                     $parm["items"][$data["begin"]."$$$"."plugin_projet".$data["id"]]["name_task"]=$fup->fields["name"];
                     ...
                     $parm["items"][$data["begin"]."$$$"."plugin_projet".$data["id"]]["name"]=$job->fields["name"];
                     $parm["items"][$data["begin"]."$$$"."plugin_projet".$data["id"]]["content"]=resume_text($job->fields["comment"],
                                                                           $CFG_GLPI["cut"]);
...
//                   }
   ...
   static function displayPlanningItem($parm) {
      ...
      echo $LANG['plugin_projet'][30]." : <br>".$parm["name_task"];
		...
		if ($parm["end"])
         echo "<strong>".$LANG['search'][9]."</strong> : ".convdatetime($parm["end"])."<br>";
