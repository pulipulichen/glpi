...
function intervaleDates($date1,$date2) { 
   ...
   // $date2 sera plus r�cente que $date1 
   ...
   if (preg_match("/([0-9]{4})([0-9]{2})([0-9]{2})/", $date1, $regs1) 
      ...
      // on ajoute les jours restant � $y1 et les jours de plus � $y2 + 1 pour le passage d'ann�e 
      // puis on ajoute les jours des ann�es entre $y1 et $y2 
      if ($y1 != $y2) { 
      ...
      // Si $y1 == $y2 
      // si les mois sont �gaux, on renvoie la diff�rence entre les jours 
      if ($m1 == $m2) 
         return $d2 - $d1; 
