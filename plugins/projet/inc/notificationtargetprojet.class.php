...
class PluginProjetNotificationTargetProjet extends NotificationTarget {
   ...
   function getAdditionalTargets($event='') {
      ...
      if ($event != 'AlertExpiredTasks') {
         $this->addTarget(PluginProjetNotificationTargetProjet::PROJET_MANAGER,$LANG['plugin_projet']['mailing'][2]);
         $this->addTarget(PluginProjetNotificationTargetProjet::PROJET_GROUP,$LANG['plugin_projet']['mailing'][13]);
         if ($event == 'newtask' || $event == 'updatetask' || $event == 'deletetask') {
            $this->addTarget(PluginProjetNotificationTargetProjet::PROJET_TASK_TECHNICIAN,$LANG['plugin_projet']['mailing'][3]);
            $this->addTarget(PluginProjetNotificationTargetProjet::PROJET_TASK_GROUP,$LANG['plugin_projet']['mailing'][11]);
            $this->addTarget(PluginProjetNotificationTargetProjet::PROJET_TASK_CONTACT,$LANG['plugin_projet']['mailing'][12]);
         }
      }
   }
   ...
   function getTaskGroupAddress ($options=array()) {
      ...
      if (isset($options['groups_id'])
         ...
         $query = $this->getDistinctUserSql().
                   " FROM `glpi_users`
                    LEFT JOIN `glpi_groups_users` ON (`glpi_groups_users`.`users_id` = `glpi_users`.`id`) 
                    LEFT JOIN `glpi_plugin_projet_tasks` ON (`glpi_plugin_projet_tasks`.`groups_id` = `glpi_groups_users`.`groups_id`)
                    WHERE `glpi_plugin_projet_tasks`.`id` = '".$options['tasks_id']."'";
   ...
   function getDatasForTemplate($event,$options=array()) {
      ...
      if ($event == 'AlertExpiredTasks') {
         ...
         foreach($options['tasks'] as $id => $task) {
            ...
            $tmp['##task.users##'] = html_clean(getUserName($task['users_id']));
            ...
            if (!empty($plans)) {
               foreach ($plans as $plan) {
                  $tmp['##task.datebegin##'] = convDateTime($plan["begin"]);
                  $tmp['##task.dateend##'] = convDateTime($plan["end"]);
               }
            } else {
               $tmp['##task.datebegin##'] = '';
               $tmp['##task.dateend##'] = '';
            ...
            $tmp['##task.comment##'] = html_clean($comment);
         ...
         $this->datas['##projet.datebegin##'] = convDate($this->obj->getField('date_begin'));
         
         $this->datas['##lang.projet.dateend##'] = $LANG['search'][9];
         $this->datas['##projet.dateend##'] = convDate($this->obj->getField('date_end'));
         
         $this->datas['##lang.projet.users##'] = $LANG['common'][34];
         $this->datas['##projet.users##'] =  html_clean(getUserName($this->obj->getField("users_id")));
         ...
         $this->datas['##projet.parent##'] =  Dropdown::getDropdownName('glpi_plugin_projet_projets',
                                                       $this->obj->getField('plugin_projet_projets_id'));
         ...
         $this->datas['##projet.comment##'] = html_clean($comment);
         ...
         $this->datas['##projet.description##'] = html_clean($description);
         ...
         if (isset($this->target_object->oldvalues) && !empty($this->target_object->oldvalues) && $event=='update') {
            ...
            if (isset($this->target_object->oldvalues['date_begin'])) {
               if (empty($this->target_object->oldvalues['date_begin']))
                  ...
                  $tmp['##update.datebegin##'] = convDate($this->target_object->oldvalues['date_begin']);
            }
            ...
            if (isset($this->target_object->oldvalues['date_end'])) {
               if (empty($this->target_object->oldvalues['date_end']))
                  ...
                  $tmp['##update.dateend##'] = convDate($this->target_object->oldvalues['date_end']);
            }
            ...
            if (isset($this->target_object->oldvalues['users_id'])) {
               if (empty($this->target_object->oldvalues['users_id']))
                  ...
                  $tmp['##update.users##'] = html_clean(getUserName($this->target_object->oldvalues['users_id']));
            }
            ...
            if (isset($this->target_object->oldvalues['comment'])) {
               if (empty($this->target_object->oldvalues['comment'])) {
                  ...
                  $tmp['##update.comment##'] = html_clean($comment);
               }
            }
            ...
            if (isset($this->target_object->oldvalues['description'])) {
               if (empty($this->target_object->oldvalues['description'])) {
                  ...
                  $tmp['##update.description##'] = html_clean($description);
               }
            }
         ...
         $restrict .= " ORDER BY `date_end` DESC";
         ...
         if (!empty($tasks)) {
            ...
            foreach ($tasks as $task) {
               ...
               $tmp['##task.sub##'] = html_clean($sub);
               $others = stripslashes(str_replace(array('\r\n', '\n', '\r'), "<br/>", $task['others']));
               $tmp['##task.others##'] = html_clean($others);
               $affect = stripslashes(str_replace(array('\r\n', '\n', '\r'), "<br/>", $task['affect']));
               $tmp['##task.affect##'] = html_clean($affect);
               $tmp['##task.parenttask##'] = Dropdown::getDropdownName('glpi_plugin_projet_tasks',
                                                          $task['plugin_projet_tasks_id']);
               $tmp['##task.gantt##'] = Dropdown::getYesNo($task['show_gantt']);
               $tmp['##task.depends##'] = Dropdown::getYesNo($task['depends']);
               $tmp['##task.realtime##'] = Ticket::getRealtime($task["realtime"]);
